...
Injects a dynamic progress bar with percentage text into the Storyline 360 course player's top bar. Displays two visible elements side by side:

- **"Course Progress: XX%"** — text label
- **Visual progress bar** — fills and changes color at 100%

A slide counter ("Slide: X/Y") is also built into the script but is hidden by default as of v1.0.1. It can be re-enabled with a single property change (see Slide Counter below).
...
2. If they don't exist (first slide), it creates the full widget: text label, background track, and fill bar.
...
The slide counter is hidden by default as of v1.0.1. To re-enable it, find the comment `/* Slide counter hidden (display:none) — change to "block" to re-enable */` in the script and change `display:none` to `display:block` in the `slideCounterText` style string on the line below it.
...
|---|---|---|
| 1.0.0 | — | Initial release with progress bar and slide counter |
